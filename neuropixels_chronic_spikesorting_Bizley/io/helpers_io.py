# -*- coding: utf-8 -*-
"""
Helpers Input/output functions for Neuropixels

Adapted from NeuroPyxels inout functions (https://github.com/m-beau/NeuroPyxels)
...
# except ImportError:
#     print('NeuroPyxels not installed, some functions will not work')
...
def metadata(dp, dp_bin=None):
    ...
    Parameters:
        ...
        - catgt_path: str, path to .bin file to read metadata from. If None, will look for .bin file in dp.
    ...
    dp = Path(dp)
    if dp_bin is None:
        dp_bin = dp.parents[0] # Look for ap.meta in parent directory
    ...
    # find meta file
    glx_ap_files = list_files(dp_bin, "ap.meta", True)
    glx_lf_files = list_files(dp_bin, "lf.meta", True)
    oe_files = list_files(dp_bin, "oebin", True)
    glx_found = np.any(glx_ap_files) or np.any(glx_lf_files)
    oe_found = np.any(oe_files)
    assert glx_found or oe_found, \
        f'WARNING no .ap/lf.meta (spikeGLX) or .oebin (OpenEphys) file found at {dp_bin}.'
...
def get_npix_sync(dp, output_binary=False, unit='seconds', sync_trial_chan=[5], verbose=True):
    ...
    assumes that the nidq.bin file is in the parent directory of dp
    Parameters:
        - dp: str, datapath
    ...
    metafile = op.join(nidqpath,metafile[0])
    ...
    if sync_dp.exists():
        if verbose: print(f"No file ending in 'on_samples.npy' with the right sampling rate found in sync_chan directory: extracting sync channel from binary.\n")
    ...
    if fname=='':
        ...
        fname = '{}_t0.nidq.bin'.format(dp.name)
...
# Commented for Nellie: now using onsets/offsets from nidq
